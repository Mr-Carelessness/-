...
public class FrmSubscribeDetailmg_add extends JFrame implements ActionListener {
	private FrmSubscribemg super_Jrame = null; 
	...
	public FrmSubscribeDetailmg_add(FrmSubscribemg frmSubscribemg, Commodity commodity) {
		// TODO Auto-generated constructor stub
		super_Jrame = frmSubscribemg;
		...
		JLabel label_4 = new JLabel("\u670D\u52A1\u6298\u6263");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == button_add) {
			...
			subscribeDetail.setCommodity_id(commodity.getId());//商品编号
			subscribeDetail.setCommodity_itsprice(commodity.getPrice());//商品单价
			subscribeDetail.setCommodity_name(commodity.getName());//商品名称
			try{
				int number = Integer.parseInt(this.textField_num.getText());
				subscribeDetail.setNumber(number);
			...
			try {
				...
				subscribeDetail.setDiscount(discount);
			} catch (NumberFormatException e1) {
			...
			subscribeDetail.setCommodity_price(subscribeDetail.getNumber()*subscribeDetail.getCommodity_itsprice()*subscribeDetail.getDiscount());
			//符合条件，则直接添加商品
			super_Jrame.subscribeDetails.add(subscribeDetail);
			super_Jrame.sumprice = super_Jrame.sumprice + subscribeDetail.getCommodity_price();
			...
			super_Jrame.reloadTable_subscribeDetail(true);
			this.dispose();
