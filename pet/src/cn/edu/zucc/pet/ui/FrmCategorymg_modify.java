...
public class FrmCategorymg_modify extends JFrame implements ActionListener {
	private JTextField textField_name;
	private JTextArea textField_descp;
	private FrmCategorymg super_Jrame = null; 
	JButton button_modify = new JButton("\u4FEE\u6539\u7C7B\u522B");
	JButton button_exit = new JButton("\u9000\u51FA");
	Category category = null;
	int selectedRow = 0;
	
	public FrmCategorymg_modify(FrmCategorymg frmCategorymg,int i) {
		this.super_Jrame = frmCategorymg;
		this.selectedRow = i;
		this.category = frmCategorymg.categorys.get(this.selectedRow);//获取对应一行的brand
		...
		//第一个
		if(category.getName() == null || category.getName().equals("") == true) {
			textField_name.setText("");
		}else {
			textField_name.setText(category.getName());
		}
		...
		if(category.getDescription() == null || category.getDescription().equals("") == true) {
			textField_descp.setText("必填");
			textField_descp.setForeground(Color.GRAY);
		}else {
			textField_descp.setText(category.getDescription());
		...
		button_modify.setBounds(216, 143, 93, 27);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == this.button_modify) {
			...
			try {
				...
				c = this.category;
				JOptionPane.showMessageDialog(null, "修改类别成功", "正确提示",JOptionPane.INFORMATION_MESSAGE);
				//所有的都要修改一遍
				super_Jrame.categorys.get(this.selectedRow).setName(c.getName());
				super_Jrame.categorys.get(this.selectedRow).setDescription(c.getDescription());
