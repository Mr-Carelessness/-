...
public class FrmServicemg extends JFrame implements ActionListener {
	private JTextField textField;
	private JTable table;
	private Object tblData[][];
	private Object tblTitle[] = {"服务编号","服务名称","类别编号","类别名称","服务品牌","服务价格","条形码"};
	List<Commodity> commoditys = null;
	DefaultTableModel tablmod = new DefaultTableModel();
	JScrollPane scrollPane = new JScrollPane();
	
	JButton button_add = new JButton(" \u65B0\u589E\u670D\u52A1 ");//新增按钮
	JButton button_modify = new JButton(" \u4FEE\u6539\u670D\u52A1 ");//修改按钮
	JButton button_delete = new JButton(" \u5220\u9664\u670D\u52A1 ");//删除按钮
	...
	public void reloadTable(boolean isUpdated){
		try {
			if(isUpdated == false) {
				...
				commoditys = (new CommodityManager()).searchCommodity(this.textField.getText(),choice_service);
			}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == this.button_search) {
			...
			//没有选择提示框直接删除
			if(i < 0) {
				JOptionPane.showMessageDialog(null, "请选择商品(服务)","错误",JOptionPane.ERROR_MESSAGE);
			}
			else {
				...
				try {
					...
					JOptionPane.showMessageDialog(null, "删除商品(服务)成功", "正确提示",JOptionPane.INFORMATION_MESSAGE);
			...
			if(this.table.getSelectedRow() >= 0) {
				FrmServicemg_modify fm = new FrmServicemg_modify(this,this.table.getSelectedRow());//获取对应一行的brand，再根据brand值修改
				fm.setVisible(true);				
			}else {
				JOptionPane.showMessageDialog(null, "请选择商品(服务)","错误",JOptionPane.ERROR_MESSAGE);
			}
		}else if(e.getSource() == this.button_add) {
			FrmServicemg_add fm = new FrmServicemg_add(this);
			fm.setVisible(true);			
